...
func convertPDFToPNG(ctx context.Context, pdfRaw []byte, dpi int, converter, tempDir string) ([]byte, error) {
	...
	trimmedPNG, trimErr := trimUniformBordersFromPNG(pngRaw)
	if trimErr == nil && len(trimmedPNG) > 0 {
		pngRaw = trimmedPNG
	}
	return pngRaw, nil
}

func trimUniformBordersFromPNG(pngRaw []byte) ([]byte, error) {
	src, _, err := image.Decode(bytes.NewReader(pngRaw))
	if err != nil {
		return nil, fmt.Errorf("decode png for trim: %w", err)
	}
	b := src.Bounds()
	width := b.Dx()
	height := b.Dy()
	...
	bg := detectDominantBorderColor(src, b)
	if bg == nil {
		return pngRaw, nil
	}
	...
	for minY <= maxY && rowMostlyMatchesColor(src, minY, minX, maxX, *bg) {
		minY++
	}
	for maxY >= minY && rowMostlyMatchesColor(src, maxY, minX, maxX, *bg) {
		maxY--
	}
	for minX <= maxX && colMostlyMatchesColor(src, minX, minY, maxY, *bg) {
		minX++
	}
	for maxX >= minX && colMostlyMatchesColor(src, maxX, minY, maxY, *bg) {
		maxX--
	}
	...
	draw.Draw(trimmed, trimmed.Bounds(), src, trimRect.Min, draw.Src)

	var out bytes.Buffer
	enc := png.Encoder{CompressionLevel: png.BestCompression}
	if err = enc.Encode(&out, trimmed); err != nil {
		return nil, fmt.Errorf("encode trimmed png: %w", err)
	}
	return out.Bytes(), nil
}

func detectDominantBorderColor(img image.Image, b image.Rectangle) *color.RGBA {
	type colorKey struct {
		r uint8
		g uint8
		b uint8
	}
	counts := map[colorKey]int{}
	samples := map[colorKey]color.RGBA{}

	record := func(c color.RGBA) {
		q := colorKey{
			r: c.R >> 3,
			g: c.G >> 3,
			b: c.B >> 3,
	...
	maxY := b.Max.Y - 1
	for x := minX; x <= maxX; x++ {
		record(colorToRGBA(img.At(x, minY)))
		record(colorToRGBA(img.At(x, maxY)))
	}
	for y := minY; y <= maxY; y++ {
		record(colorToRGBA(img.At(minX, y)))
		record(colorToRGBA(img.At(maxX, y)))
	}
...
func rowMostlyMatchesColor(img image.Image, y, minX, maxX int, target color.RGBA) bool {
	const (
		maxDeltaPerChannel = 14
		minMatchRatio      = 0.992
	)
	width := maxX - minX + 1
	if width <= 0 {
	...
	matches := 0
	for x := minX; x <= maxX; x++ {
		if colorNear(colorToRGBA(img.At(x, y)), target, maxDeltaPerChannel) {
			matches++
		}
	}
...
func colMostlyMatchesColor(img image.Image, x, minY, maxY int, target color.RGBA) bool {
	const (
		maxDeltaPerChannel = 14
		minMatchRatio      = 0.992
	)
	...
	return float64(matches)/float64(height) >= minMatchRatio
}

func colorNear(a, b color.RGBA, maxDelta uint8) bool {
	return absInt(int(a.R)-int(b.R)) <= int(maxDelta) &&
		absInt(int(a.G)-int(b.G)) <= int(maxDelta) &&
		absInt(int(a.B)-int(b.B)) <= int(maxDelta)
}
