...
- `WF21_SUMMARY_ERROR_LINK_URL` (default empty; when set, sends fallback text with this clickable link if summary image export/send fails)
- `WF21_SUMMARY_ERROR_LINK_LABEL` (default `Backlogs per hub level:`; markdown bold label shown above fallback link)
